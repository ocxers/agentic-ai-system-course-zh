...
一次工具调用 (tool call) 是每个 agent 的原子单位。模型发出一个结构化请求,说明要用哪些参数调用哪个函数;你的代码运行它;结果作为另一条消息回送;模型写出最终回答。还没有循环、没有记忆、没有编排 —— 就一次往返。本章要讲的,就是如何把这一次往返做到分毫不差。课程其余部分,无非是在这同一个机制上做变种、往上叠。
...
函数调用 (function calling) 不会让模型变得更聪明。它给模型开了一条路,让它能**向你的代码索取**那些它自己办不到的事。模型决定 *何时* 索取、*用什么参数* 索取;真正的活儿发生在你能保证正确性的地方 —— 你的代码里。一旦你把这个分工想透,你写出的工具会更好,做废的 agent 会更少。
...
一旦出岔子,排查就归结为一个诊断问题:是主厨把单子写错了,还是后厨把菜做错了?两种不同的故障,两种不同的修法。一旦你能在脑子里把这两者分开,debug 就不再像在变魔术。
...
无论工具是去取天气、查你的数据库,还是跑一条 shell 命令,协议都一样。传输格式(wire format)不关心工具到底做了什么;只有你关心。
...
- **托管(内置)工具。** 由 provider 自己提供并执行、而非你来执行的工具 —— web search、code execution、file search、computer use。传输层上的 schema 和 tool-use 形态看起来一模一样,但结果回来时,你的派发代码根本没跑过。取舍:集成更简单,但对"跑什么、怎么计费"的控制更少。
...
传输格式和具体字段名会变,概念却是稳定的。真要接入它们中的任何一个,当天让你的 agent 把对应 provider 的最新文档拉出来看一眼。
...
如果你是认真在做,把传输格式藏到一个小 adapter 后面,这样你的工具就不必在乎你上周用的是哪家 provider。OpenCode 和 Hermes Agent 都正是这么做的;让你的 agent 给你的技术栈搭一个脚手架。
