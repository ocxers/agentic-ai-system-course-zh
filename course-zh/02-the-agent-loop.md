...
传输格式因 provider 而异;但循环对响应形态的处理 —— 累积、验证、派发或浮出 —— 各家都一样。

### 中断与取消

用户按 Ctrl-C、超时触发、父进程认为循环跑得太久 —— 这些都需要向内传播。模式是:每个循环都持有一个 abort token,每个长时间运行的步骤都检查它,被触发的 token 会干净地展开为一个 partial 结果,而不是把整个进程拉倒。"中断恰好在工具调用进行中到达" 这种情况值得单独想一想:让工具跑完(或者让工具自己去检查 token),而不要把一个写到一半的操作丢下不管、变成孤儿状态。
