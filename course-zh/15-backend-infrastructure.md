...
- **SQLite + WAL**(OpenCode、Hermes CLI) — 嵌入式;数据就在进程旁边;备份就是复制文件。适合嵌入式和单机。Hermes Agent 的 `apply_wal_with_fallback` 在 NFS/SMB 场景下(WAL 不兼容)会回落到 journal 模式来处理。
...
多租户分行和加密密钥只是安全故事的一部分,不是全部。后端一旦开始处理真用户的真数据,会冒出来五个关注点:
...
- **加密。** 传输中(边缘要 TLS,*内部服务之间也要*,不只是在负载均衡器那一层)以及静态时(状态存储用数据库级加密;message 正文或 memory 条目这类高敏感列用字段级加密)。磁盘级加密是底线,不是天花板。
...
这五条是后端对监管者、安全团队、用户的契约。第 18 章覆盖端到端威胁模型;本章是 *实现* 这个契约所做的存储与路由决策的归属地。
...
- **10 台服务器(大致量级)。** Postgres 争用会开始咬人 — `SELECT ... FOR UPDATE` 锁等待、连接池饱和。具体阈值取决于 schema、索引策略、锁粒度、读写比例;有些负载更早就感受到,有些撑到几百台之前都没事。修法:连接池(PgBouncer)、把队列分区(按租户或队列名分片)、改用 `SELECT ... FOR UPDATE SKIP LOCKED` 让 worker 不会互相串行。
- **100 台服务器。** Postgres 当队列已经不够用了。修法:专用的分布式队列(Kafka、Redis Streams、NATS JetStream)。
- **1000+ 台服务器。** 调度器心跳自身成了扩展问题。修法:按租户或区域给调度器分片。

实话:多数 agent 后端永远到不了 10 台服务器。还没到 10 台就为 1000 台优化,是把工程当娱乐玩。
...
账本是持久化的(第 8 章),所以 message log 部分压缩不会丢成本数据。把账本和 trace 流水线配对(第 16 章);成本是每租户层面最有用的可绘制信号之一。
...
- **outbox 本身也是扩展性问题。** outbox 堆积是下游 API 比你 agent 吞吐慢的信号。监控 outbox 深度;增长时告警。
...
- **基于文件的 session 存储** 用于所有事情。磁盘上的 JSON 文件挺好,直到一万个文件挤在同一个目录、文件系统索引器卡死。session 数量过了几千之后,用数据库。
